...
@bot.command()
async def up(ctx, option: int, *stats):
    """
    Commande pour améliorer les statistiques selon différentes options :
    Option 1 : +0.1 pour 4 statistiques.
    Option 2 : +0.2 pour 2 statistiques.
    Option 3 : +0.2 pour 1 statistique et +0.1 pour 2 statistiques.
    Option 4 : +0.3 pour 1 statistique et +0.1 pour 1 statistique.
    Option 5 : +0.4 pour 1 statistique.
    """
    try:
        # Convertir les statistiques en liste de flottants
        stats = list(map(float, stats))

        # Vérifier qu'il y a assez de statistiques pour l'option choisie
        if option == 1 and len(stats) >= 4:
            for i in range(4):
                if stats[i] + 0.1 > 100:
                    await ctx.send(f"Statistique {i+1} ne peut pas dépasser 100. Action annulée.")
                    return
                stats[i] += 0.1
        elif option == 2 and len(stats) >= 2:
            for i in range(2):
                if stats[i] + 0.2 > 100:
                    await ctx.send(f"Statistique {i+1} ne peut pas dépasser 100. Action annulée.")
                    return
                stats[i] += 0.2
        elif option == 3 and len(stats) >= 3:
            if stats[0] + 0.2 > 100:
                await ctx.send("Statistique 1 ne peut pas dépasser 100. Action annulée.")
                return
            stats[0] += 0.2
            for i in range(1, 3):
                if stats[i] + 0.1 > 100:
                    ...
                    return
                stats[i] += 0.1
        elif option == 4 and len(stats) >= 2:
            if stats[0] + 0.3 > 100:
                await ctx.send("Statistique 1 ne peut pas dépasser 100. Action annulée.")
                return
            stats[0] += 0.3
            if stats[1] + 0.1 > 100:
                await ctx.send("Statistique 2 ne peut pas dépasser 100. Action annulée.")
                return
            stats[1] += 0.1
        elif option == 5 and len(stats) >= 1:
            if stats[0] + 0.4 > 100:
                await ctx.send("Statistique 1 ne peut pas dépasser 100. Action annulée.")
                return
            stats[0] += 0.4
        else:
            await ctx.send("Pas assez de statistiques fournies pour cette option.")
            return

        # Créer un message avec les statistiques mises à jour
        embed = discord.Embed(title="Stats mises à jour", color=0xffa500)
        for i, stat in enumerate(stats, start=1):
            embed.add_field(name=f"Statistique {i}", value=f"{stat:.1f}", inline=False)

        await ctx.send(embed=embed)

    except ValueError:
        await ctx.send("Veuillez fournir uniquement des nombres valides comme statistiques.")
